...
export function SmartLabelEditor() {
  ...
  const handleBackfill = useCallback(async () => {
    ...
    try {
      ...
      setBackfillResult(t("smartLabelEditor.appliedLabels", { count }));
    } catch (err) {
    ...
    <div className="space-y-3">
      {rules.length > 0 && (
        ...
        >
          ...
          {backfilling ? t("smartLabelEditor.applyingToExisting") : t("smartLabelEditor.applyToExisting")}
        </button>
      )}
        ...
        <div className="border border-border-primary rounded-md p-3 space-y-3">
          ...
          <div>
            ...
            >
              ...
              {t("smartLabelEditor.optionalFilterCriteria")}
            </button>
            ...
            {showCriteria && (
              <div className="mt-2 space-y-1.5">
                <TextField
                  ...
                  placeholder={t("smartLabelEditor.fromContains")}
                />
                <TextField
                  type="text"
                  value={criteriaTo}
                  onChange={(e) => setCriteriaTo(e.target.value)}
                  placeholder={t("smartLabelEditor.toContains")}
                />
                <TextField
                  type="text"
                  value={criteriaSubject}
                  onChange={(e) => setCriteriaSubject(e.target.value)}
                  placeholder={t("smartLabelEditor.subjectContains")}
                />
                <TextField
                  type="text"
                  value={criteriaBody}
                  onChange={(e) => setCriteriaBody(e.target.value)}
                  placeholder={t("smartLabelEditor.bodyContains")}
                />
