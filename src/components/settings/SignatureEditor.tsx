...
export function SignatureEditor() {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({ placeholder: t("settings.signatures.writePlaceholder") }),
    ],
    content: "",
    editorProps: {
      attributes: {
      ...
      {showForm ? (
        <div className="border border-border-primary rounded-md p-3 space-y-2">
          <TextField
            ...
            placeholder={t("settings.signatures.signatureName")}
          />
          <div className="border border-border-primary rounded overflow-hidden bg-bg-tertiary">
            <div className="flex items-center justify-between">
              {isHtmlMode ? (
                <span className="px-2 py-1 text-xs text-text-secondary">{t("settings.signatures.htmlSource")}</span>
              ) : (
              ...
              <button
                ...
                title={isHtmlMode ? t("settings.signatures.switchToVisual") : t("settings.signatures.editHtml")}
              >
                <Code size={14} />
              </button>
            </div>
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-1.5 text-xs text-text-secondary">
              <input
                ...
                className="rounded"
              />
              {t("settings.signatures.setAsDefault")}
            </label>
          </div>
          <div className="flex items-center gap-2">
            <button
