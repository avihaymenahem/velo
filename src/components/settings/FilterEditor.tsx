  ...
  const filterDescriptions = useMemo(() => {
    ...
    for (const filter of filters) {
      try {
        ...
        map.set(filter.id, parts.join(", ") || t("settings.filters.noCriteria"));
      } catch {
        map.set(filter.id, t("settings.filters.invalidCriteria"));
      }
    }
    return map;
  }, [filters]);
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-text-tertiary truncate">
              {filterDescriptions.get(filter.id) ?? t("settings.filters.noCriteria")}
            </div>
          </div>
      ...
      {showForm ? (
        <div className="border border-border-primary rounded-md p-3 space-y-3">
          <TextField
            ...
            placeholder={t("settings.filters.filterName")}
          />

          <div>
            <div className="text-xs font-medium text-text-secondary mb-1.5">{t("settings.filters.matchCriteria")}</div>
            <div className="space-y-1.5">
              <TextField
                type="text"
                value={criteriaFrom}
                onChange={(e) => setCriteriaFrom(e.target.value)}
                placeholder={t("settings.filters.fromContains")}
              />
              <TextField
                type="text"
                value={criteriaTo}
                onChange={(e) => setCriteriaTo(e.target.value)}
                placeholder={t("settings.filters.toContains")}
              />
              <TextField
                type="text"
                value={criteriaSubject}
                onChange={(e) => setCriteriaSubject(e.target.value)}
                placeholder={t("settings.filters.subjectContains")}
              />
              <TextField
                type="text"
                value={criteriaBody}
                onChange={(e) => setCriteriaBody(e.target.value)}
                placeholder={t("settings.filters.bodyContains")}
              />
              <label className="flex items-center gap-1.5 text-xs text-text-secondary">
                <input
                  ...
                  className="rounded"
                />
                {t("settings.filters.hasAttachment")}
              </label>
            </div>
          </div>

          <div>
            <div className="text-xs font-medium text-text-secondary mb-1.5">{t("settings.filters.actions")}</div>
            <div className="space-y-1.5">
              {labels.length > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-text-secondary w-20">{t("settings.filters.applyLabel")}</span>
                  <select
                    value={actionLabel}
