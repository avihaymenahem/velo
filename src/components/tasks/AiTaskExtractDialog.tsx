...
const PRIORITY_OPTIONS: { value: TaskPriority; key: string; color: string }[] = [
  { value: "none", key: "common.none", color: "text-text-tertiary" },
  { value: "low", key: "tasks.low", color: "text-blue-400" },
  { value: "medium", key: "tasks.medium", color: "text-amber-400" },
  { value: "high", key: "tasks.high", color: "text-orange-500" },
  { value: "urgent", key: "tasks.urgent", color: "text-red-500" },
];
      ...
      <div className="relative glass-modal rounded-xl shadow-2xl w-[480px] max-w-[90vw] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border-secondary">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-text-primary">{t("tasks.createFromEmail")}</h3>
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              ...
              <p className="text-sm text-text-secondary">{t("tasks.extracting")}</p>
            </div>
          ) : error && !title ? (
            ...
            <>
              {/* Title */}
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-1.5">{t("calendar.title")}</label>
                <input
                  type="text"
                  value={title}
              ...
              {/* Description */}
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-1.5">{t("tasks.description")}</label>
                <textarea
                  value={description}
