...
export function WeekView({ currentDate, events, onEventClick }: WeekViewProps) {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
        ...
        {days.map((day, i) => {
          ...
          return (
            <div key={i} className="px-2 py-2 text-center border-r border-border-secondary">
              <div className="text-xs text-text-tertiary">{t(`calendar.days.${DAY_NAME_KEYS[day.getDay()]}`)}</div>
      ...
      {/* All-day events row */}
      <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-border-primary shrink-0">
        <div className="border-r border-border-secondary px-1 py-1 text-[0.625rem] text-text-tertiary">{t("calendar.allDay").toLowerCase()}</div>
        {days.map((day, i) => {
