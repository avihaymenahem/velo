...
export function CalendarReauthBanner({ accountId, email, onReauthSuccess }: CalendarReauthBannerProps) {
  ...
  const handleReauthorize = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("calendarReauth.reauthorize"));
      setStatus("error");
    }
  };
  ...
  return (
    <div className="mx-6 my-4 p-4 rounded-lg bg-warning/10 border border-warning/30 flex items-start gap-3">
      ...
      <div className="flex-1">
        <p className="text-sm font-medium text-text-primary">{t("calendarReauth.title")}</p>
        <p className="text-xs text-text-secondary mt-1">
          {t("calendarReauth.description")}
        </p>
        {error && (
        ...
        >
          ...
          {status === "authorizing" ? t("calendarReauth.waitingForAuth") : t("calendarReauth.reauthorize")}
        </button>
      </div>
    </div>
  );
