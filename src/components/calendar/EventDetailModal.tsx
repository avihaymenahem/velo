...
export function EventDetailModal({ event, calendars, accountId, onClose, onUpdated }: EventDetailModalProps) {
  ...
  if (editing) {
    return (
      <Modal isOpen={true} onClose={onClose} title={t("eventDetail.editEvent")} width="w-full max-w-md">
        <div className="p-4 space-y-3">
          <TextField
            label={t("eventDetail.title")}
            type="text"
            value={summary}
          ...
          <div className="grid grid-cols-2 gap-3">
            <TextField
              label={t("eventDetail.start")}
              type="datetime-local"
              value={startTime}
              onChange={(e) => setStartTime(e.target.value)}
            />
            <TextField
              label={t("eventDetail.end")}
              type="datetime-local"
              value={endTime}
          ...
          <div>
            <label className="text-xs text-text-secondary block mb-1">{t("eventDetail.description")}</label>
            <textarea
              value={description}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button variant="primary" size="md" onClick={handleSave} disabled={saving || !summary.trim()}>
              {saving ? t("eventDetail.saving") : t("common.save")}
            </Button>
          </div>
        </div>
      </Modal>
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title={event.summary ?? t("eventDetail.event")} width="w-full max-w-md">
      <div className="p-4 space-y-3">
        {calendar && (
        ...
        {attendees.length > 0 && (
          <div className="border-t border-border-primary pt-3">
            <div className="text-xs text-text-tertiary mb-1.5">{t("eventDetail.attendees")}</div>
        ...
        <div className="flex justify-between pt-2 border-t border-border-primary">
          {confirmDelete ? (
            <div className="flex items-center gap-2">
              <span className="text-xs text-danger">{t("eventDetail.deleteConfirm")}</span>
              <Button variant="danger" size="xs" onClick={handleDelete} disabled={deleting}>
                {deleting ? t("eventDetail.deleting") : t("eventDetail.yesDelete")}
              </Button>
