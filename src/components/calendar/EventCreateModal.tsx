    ...
    <Modal isOpen={true} onClose={onClose} title={t("calendar.createEvent")} width="w-full max-w-md">
      <form onSubmit={handleSubmit} className="p-4 space-y-3">
        ...
        {calendars && calendars.length > 1 && (
          <div>
            <label className="text-xs text-text-secondary block mb-1">{t("calendar.calendarLabel")}</label>
            <select
              value={calendarId}
            ...
            >
              {calendars.map((cal) => (
                <option key={cal.id} value={cal.id}>
                  {cal.display_name ?? t("calendar.calendarLabel")}
                  {cal.is_primary ? ` (${t("common.primary")})` : ""}
                </option>
              ))}
            </select>
          </div>
        ...
        <div>
          <label className="text-xs text-text-secondary block mb-1">{t("calendar.descriptionLabel")}</label>
          <textarea
            value={description}
