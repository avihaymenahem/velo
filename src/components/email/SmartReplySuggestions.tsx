  ...
  return (
    <div className="mx-4 my-2 p-3 rounded-lg bg-accent/5 border border-accent/20">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-medium text-accent flex-1">{t("ai.quickReplies")}</span>
        <button
          onClick={handleRefresh}
          className="p-0.5 text-text-tertiary hover:text-accent transition-colors"
          title={t("ai.refreshSuggestions")}
        >
      ...
      {loading && !replies && (
        <div className="flex items-center gap-2 text-text-tertiary">
          ...
          <span className="text-xs">{t("ai.generatingSuggestions")}</span>
        </div>
      )}
      {replies && (
