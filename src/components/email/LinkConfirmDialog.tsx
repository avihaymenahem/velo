  ...
  const customHeader = (
    <div className={`px-4 py-3 ${headerBg} flex items-center gap-2.5 rounded-t-lg`}>
      ...
      <h2 className={`text-sm font-semibold ${headerText}`}>
        {isHigh ? t("phishing.highRiskLink") : t("phishing.suspiciousLink")}
      </h2>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 space-y-3">
        {/* URL display */}
        <div>
          <label className="text-xs text-text-tertiary block mb-1">{t("phishing.fullUrl")}</label>
        ...
        {linkAnalysis.displayText && (
          <div>
            <label className="text-xs text-text-tertiary block mb-1">{t("phishing.linkText")}</label>
        ...
        {linkAnalysis.triggeredRules.length > 0 && (
          <div>
            <label className="text-xs text-text-tertiary block mb-1.5">
              {t("phishing.issuesDetected")} ({linkAnalysis.triggeredRules.length})
            </label>
