        ...
        {expanded && (
          <div className="mt-1 text-xs text-text-tertiary">
            {message.to_addresses && (
              <span>{t("message.to")} {message.to_addresses}</span>
            )}
          </div>
        )}
      </button>
      ...
      {expanded && (
        <div className="px-4 pb-4">
          ...
          {blockImages != null ? (
            ...
            <div className="py-8 text-center text-text-tertiary text-sm">{t("common.loading")}</div>
          )}
...
function UnsubscribeLink({
  ...
  return (
    ...
    >
      <MailMinus size={12} />
      {status === "loading" && t("message.unsubscribing")}
      {status === "done" && t("message.unsubscribed")}
      {status === "failed" && t("message.unsubscribeFailed")}
      {status === "idle" && t("message.unsubscribe")}
    </button>
  );
}
