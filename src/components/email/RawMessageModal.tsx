          ...
          <div className="flex items-center gap-2">
            {raw && (
              <button
                ...
                title={t("rawMessage.copyToClipboard")}
              >
                {copied ? <Check size={14} /> : <Copy size={14} />}
                {copied ? t("rawMessage.copied") : t("rawMessage.copy")}
              </button>
            )}
            <button
              onClick={onClose}
      ...
      <div className="max-h-[70vh] overflow-y-auto p-4">
        {loading && (
          <div className="flex items-center justify-center py-12 text-text-tertiary text-sm">
            {t("rawMessage.loadingSource")}
          </div>
        )}
        {error && (
          <div className="text-danger text-sm py-4">
            {t("rawMessage.failedToLoad", { error })}
          </div>
        )}
        {raw && (
