...
export function buildSearchQuery(
  ...
  if (excludeSystemLabels && !parsed.label) {
    whereClauses.push(
      `NOT EXISTS (SELECT 1 FROM thread_labels tl2 JOIN labels l2 ON l2.account_id = tl2.account_id AND l2.id = tl2.label_id WHERE tl2.account_id = m.account_id AND tl2.thread_id = m.thread_id AND UPPER(l2.name) IN ('TRASH', 'SPAM', 'DRAFT'))`,
    );
  }
